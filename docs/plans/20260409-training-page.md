...
- [x] Список наборов (из `useWordSets()`): каждый элемент показывает название и `wordCount` (общее кол-во слов)
- [x] Опция «Все слова» — всегда первой, без счётчика или с суммой всех слов
- [x] Выделение выбранного набора (состояние `selectedSetId` пробрасывается из контейнера)
- [x] Кнопка «Начать тренировку» — вызывает `onStart(selectedSetId)`
- [x] Если `loadError` (0 слов после загрузки): показать сообщение «Все слова освоены, выберите другой набор»
- [x] Написать unit-тесты: рендер с наборами, рендер без наборов, выбор набора, кнопка Start
...
- [x] Показать слово (крупно, по центру)
- [x] Локальный `showTranslation: boolean` (начальное значение `false`, сбрасывать при смене `word.id`)
- [x] Кнопка «Показать перевод» → `setShowTranslation(true)`
- [x] При `showTranslation`: показать перевод и кнопки «Знаю» (зелёная) и «Не знаю» (красная)
- [x] Кнопки «Знаю»/«Не знаю»: disabled пока выполняется предыдущая мутация (предотвратить двойной клик)
- [x] Прогресс-бар: `(currentIndex + 1) / total` с текстом «X из N»
- [x] Показать `knowledgeLevel` слова (цветной индикатор 0–5)
- [x] Написать unit-тесты: скрытый/видимый перевод, клик по кнопкам, disabled при загрузке
